...
if [ $stage -le 2 ]; then
  ...
  echo "$0: preparing text for LM training..."
  ...
  echo "$0: preparing extra corpus for sentencepiece model training..."
  if [ -f $wordlmdatadir/fisher_text0 ]; then
    rm -rf $wordlmdatadir/fisher_text0
  fi
  for x in $fisher_dirs; do
    [ ! -d $x/data/trans ] \
      && "Cannot find transcripts in Fisher directory $x" && exit 1;
    cat $x/data/trans/*/*.txt | \
      grep -v "^#" | grep -v "^$" | cut -d" " -f4- >> $wordlmdatadir/fisher_text0
  done
  cat $wordlmdatadir/fisher_text0 | local/fisher_map_words.pl | \
    sed 's/^[ \t]*//'> $wordlmdatadir/fisher_text

  echo "$0: training sentencepiece model..."
  cat $wordlmdatadir/$train_set $wordlmdatadir/fisher_text > data/lang/input
  python3 ../../scripts/spm_train.py --bos_id=-1 --pad_id=0 --eos_id=1 --unk_id=2 --input=data/lang/input \
    ...
    --input_sentence_size=10000000 --treat_whitespace_as_suffix=true \
  ...
  echo "$0: preparing text and making a word dictionary for word LM..."
  cat data/lang/input > $wordlmdatadir/train
...
if [ $stage -le 3 ]; then
  ...
  if ! $use_wordlm; then
    echo "$0: binarizing subword text..."
    ...
    for dataset in $test_set; do test_paths="$test_paths $wordlmdatadir/$dataset"; done
    test_paths=$(echo $test_paths | awk '{$1=$1;print}' | tr ' ' ',')
    ${decode_cmd} $lmdatadir/logs/preprocess.log \
      python3 ../../preprocess.py --task language_modeling_for_asr \
        --workers 50 --srcdict $lmdict --only-source \
...
if [ ${stage} -le 6 ] && $use_wordlm; then
  ...
  CUDA_VISIBLE_DEVICES=$free_gpu python3 ../../train.py $wordlmdatadir --seed 1 \
    ...
    --log-interval 500 --log-format simple \
    --num-workers 0 --max-tokens 6400 --max-sentences 1024 \
    --valid-subset $valid_subset --max-sentences-valid 2048 \
    --distributed-world-size $ngpus --distributed-rank 0 --distributed-port 100 \
    --max-epoch 25 --optimizer adam --lr 0.001 --clip-norm 1.0 \
    --lr-scheduler reduce_lr_on_plateau --lr-shrink 0.5 \
    --save-dir $wordlmdir --restore-file checkpoint_last.pt --save-interval-updates 500 \
    --keep-interval-updates 3 --keep-last-epochs 5 --validate-interval 1 \
    --arch lstm_wordlm_swbd --criterion cross_entropy --sample-break-mode eos 2>&1 | tee $log_file
fi
...
if [ ${stage} -le 7 ] && $use_wordlm; then
  ...
  test_set_array=($test_set)
  for i in $(seq 0 $num); do
    log_file=$wordlmdir/logs/evaluation_${test_set_array[$i]}.log
    python3 ../../eval_lm.py $wordlmdatadir \
      --task language_modeling_for_asr --dict $wordlmdict --gen-subset ${gen_set_array[$i]} \
      --max-tokens 12800 --max-sentences 1536 --sample-break-mode eos \
...
if [ $stage -le 8 ]; then
  ...
  CUDA_VISIBLE_DEVICES=$free_gpu speech_train.py --seed 1 \
    ...
    --max-epoch 45 --optimizer adam --lr 0.001 --weight-decay 0.0 --clip-norm 2.0 \
...
if [ $stage -le 9 ]; then
  ...
  if $lm_shallow_fusion; then
    if ! $use_wordlm; then
      ...
      opts="$opts --word-dict $wordlmdict --spm-model $sentencepiece_model --lm-weight 0.9 --oov-penalty 1e-7 --coverage-weight 0.0"
