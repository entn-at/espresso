...
if [ ${stage} -le 2 ]; then
  ...
  cat $wordlmdatadir/$train_set $wordlmdatadir/train_others > data/lang/input
  ...
  echo "$0: tokenizing extra corpus for subword LM training and making a subword dictionary..."
  cat $wordlmdatadir/train_others | \
    python3 ../../scripts/spm_encode.py --model=${sentencepiece_model}.model --output_format=piece | \
...
if [ ${stage} -le 9 ]; then
  ...
  if $lm_shallow_fusion; then
    if ! $use_wordlm; then
      ...
      opts="$opts --word-dict $wordlmdict --spm-model $sentencepiece_model --lm-weight 0.9 --oov-penalty 1e-7 --coverage-weight 0.0 --eos-factor 1.5"
